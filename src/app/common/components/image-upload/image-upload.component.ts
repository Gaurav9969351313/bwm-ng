    ...
    if ((file = event.target.files[0]) && (file.type === 'image/png' || file.type === 'image/jpeg')) {
      ...
      img.onload = function() {
        if (this.width > FileSnippet.IMAGE_SIZE.width && this.height > FileSnippet.IMAGE_SIZE.height) {
          ...
          self.toastr.error(`Minimum file size is  ${FileSnippet.IMAGE_SIZE.width}x${FileSnippet.IMAGE_SIZE.height} pixels`, 'Error!');
        }
      };

      img.src = URL.createObjectURL(file);
    } else {
      this.toastr.error('Unsupported File Type. Only jpeg and png files are allowed!', 'Error!');
    }
  }
